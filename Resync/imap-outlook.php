...
//        die;
    ...
    public function arraySearch($sentImap,$arr,$index,$val,$flag = array())
    {        
        foreach($arr as $key=>$row){
            if($row[$index]==$val){
                ...
                if($row['next'] && !$row['branch']){
                    static $i =0;
                   $msg_id = $this->fetchMessageId($sentImap,$row['num']); 
                   $flag[$row['num']] =  $msg_id;
                   $next = $row['next'];
                   $i++;
                   $msg_id = $this->fetchMessageId($sentImap,$arr[$next]['num']); 
                    $flag[$arr[$next]['num']] =  $msg_id;
                   if($arr[$next]['next'] == 0 && $arr[$next]['branch'] == 0)
                   {
                       return $flag;
                   } else {
                       return $this->arraySearch($sentImap,$arr,$index,$arr[$next]['num'],$flag);
                   }
                } else {
                $msg_id = $this->fetchMessageId($sentImap,$row['num']);
                $flag[$row['num']] = $msg_id;
                ...
                {   
                    if($arr[$i]['branch'] == 0){
                        $msg_id = $this->fetchMessageId($sentImap,$arr[$i]['num']); 
                        $flag[$arr[$i]['num']] =  $msg_id;
                    }
                   if($arr[$i]['branch'] == 0 && $arr[$i]['next'] == 0){
                       break;
                   }
                }
                }
    ...
    public function fetchMessageId($sentImap,$uid)
    {
        $res = array();
        ...
        foreach ($result as $data) {            
            $res['msg_id'] = htmlspecialchars($data->message_id); 
       }
